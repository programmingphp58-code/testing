...
if (isset($_POST['credit'])) {
    ...
    if ($payment_account  == "current_account") {
        ...
        $tBalance = floatval($amount) + floatval($result['current_balance'] ?? 0);
        ...
        $BBalance = floatval($amount) + floatval($result['business_balance'] ?? 0);
        $sql = "UPDATE accounts SET business_balance=:business_balance WHERE internetid=:internetid";
        $stmt = $conn->prepare($sql);
        $stmt->execute([
            'business_balance' => $BBalance,
            'internetid' => $internetid
        ]);
    } else {

        $SBalance = floatval($amount) + floatval($result['savings_balance'] ?? 0);
    ...
    if (true) {
        ...
        $user_balance = floatval($result['current_balance']) + floatval($result['savings_balance']) + floatval($result['business_balance'] ?? 0);
    ...
    if ($payment_account == "savings_account" and floatval($amount) > floatval($result['savings_balance'] ?? 0)) {
        toast_alert('error', 'Insufficient Savings Balance');
    } elseif ($payment_account == "current_account" and floatval($amount) > floatval($result['current_balance'] ?? 0)) {
        toast_alert('error', 'Insufficient Current Balance');
    } elseif ($payment_account == "business_account" and floatval($amount) > floatval($result['business_balance'] ?? 0)) {
        ...
        if ($payment_account  == "current_account") {
            ...
            $tBalance = floatval($result['current_balance'] ?? 0) - floatval($amount);
            $sql = "UPDATE accounts SET current_balance=:current_balance WHERE internetid=:internetid";
            $stmt = $conn->prepare($sql);
            $stmt->execute([
                'current_balance' => $tBalance,
            ...
            $BBalance = floatval($result['business_balance'] ?? 0) - floatval($amount);
            $sql = "UPDATE accounts SET business_balance=:business_balance WHERE internetid=:internetid";
            $stmt = $conn->prepare($sql);
            $stmt->execute([
                'business_balance' => $BBalance,
            ...
            $SBalance = floatval($result['savings_balance'] ?? 0) - floatval($amount);
            $sql = "UPDATE accounts SET savings_balance=:savings_balance WHERE internetid=:internetid";
            $stmt = $conn->prepare($sql);
            $stmt->execute([
                'savings_balance' => $SBalance,
        ...
        if (true) {
            ...
            $full_name = $result['firstname'] . " " . $result['lastname'];
            $user_balance = floatval($result['current_balance'] ?? 0) + floatval($result['savings_balance'] ?? 0) + floatval($result['business_balance'] ?? 0);
